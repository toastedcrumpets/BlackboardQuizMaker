...
#                      })
    ...
    with package.createTest("MyTest") as test:
        #Then create pools in the package
        with test.createPool('Unique questions (in a test)', description="", instructions="", test=test) as pool:
            pool.addNumQ('Test question', 'What is the number four in arabic numerals?', 4, erramt=0.1, positive_feedback="Good!", negative_feedback="Duh?")

        #You can also instead create pools like above, then use the
        #test.add_pool function to add a single pool to multiple tests.
