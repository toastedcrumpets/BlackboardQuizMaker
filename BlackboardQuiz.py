...
class Quiz:
    def __init__(self, quizname, description_text="Created by BlackboardQuiz!"):
        """Initialises a quiz
        """
    ...
    def render_latex(self, formula, fontsize=6, dpi=300, format_='png'):
        """Renders LaTeX expression into an image data.
        """
        fig = pylab.figure()
        text = fig.text(0, 0, u'${}$'.format(formula), fontsize=fontsize)
        #Fake render to force pylab to determine the actual size of the text
        buffer_ = StringIO()
    ...
    def process_latex(self, in_string):
        ...
        for i in range(1, len(in_string), 2):
            in_string[i] = self.embed_latex(in_string[i])
    ...
    def addQuestion(self, text, answers, correct, positive_feedback="Good work", negative_feedback="That's not correct"):
        ...
        etree.SubElement(gradable, 'FEEDBACK_WHEN_CORRECT').text = positive_feedback
        etree.SubElement(gradable, 'FEEDBACK_WHEN_INCORRECT').text = negative_feedback
